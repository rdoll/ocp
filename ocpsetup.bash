...
# ocpsetup.bash
#
# Sets up the development environment for OCP
#
# Because the dev env setup includes aliases and functions (which are never
# passed to child shells), this script does a little trickery to get the
# necessary aliases and functions defined in an interactive shell. As such,
# this only supports the bash shell. Modification for other shells shouldn't
# be that hard, tho.
#


# Helper function to handle fatal errors
function ocpsetupFatal {
    echo -e "\n*** ocpsetup error: $@\n"
    exit 1
}


# What we do depends on if we are an interactive shell or not...
if [ -z "$PS1" ] ; then
    # We are NOT running in an interactive shell

    # If we are already setup, don't allow a nested one
    if [ ! -z "$OCP_VER" ] ; then
        ocpsetupFatal "You are already set up for OCP version $OCP_VER!"
    fi

    # Ensure exec failures are reported back to us instead of exiting the shell
    shopt -s execfail

    # Just in case, ensure the shell we will exec is a viable executable
    execshell="$BASH"
    if [ -x "$execshell" ] ; then
        # Exec an interactive shell (replacing this non-interactive one)
        # using this file as the rc file read for initialization
        exec $execshell --rcfile "$0" -i

        # We can only reach here if the exec failed
        ocpsetupFatal "Exec of $execshell failed with status $?"
    else
        ocpsetupFatal "'$execshell' is not an executable file"
    fi
fi


#
# We are running in an interactive shell
# This is our second pass as spawned from the first pass above
#

# Start by pulling in the user's initialization file
bashrc="$HOME/.bashrc"
if [ -f "$bashrc" ] ; then
...
#
# All OCP development environment setup
#
...
if [ ".${PWD#$OCP_ROOT/}" = ".$PWD" ] ; then
    ocpsetupFatal "Can only setup while under a version subdir of $OCP_ROOT."
fi
...
function ocpValidateVersions {
    ...
    if [ ".$OCP_VER" != ".$_srcver" ] ; then
        echo "OCP Error: Source version $_srcver does not match directory" \
            "version $OCP_VER."
        unset _srcver
        return 1
    fi
...
# Note: Does not follow links so dojotoolkit is not found
...
# Count lines of code mertics for source text files
function countloc {
  perl -e '
...
alias countcwd='countloc `find_text_files .`'
alias countsrc='countloc $OCP_SRC'
...
#
# All setup information was defined
#

# Tweak the prompt to remind the user setup was done
export PS1="OCP: $PS1"

# When this shell exits, remind the user that OCP setup has been lost
trap 'echo -e "\nExitted ocpsetup $OCP_VER development environment.\n"' 0

# Setup is complete!
echo -e "\nocpsetup for version $OCP_VER completed.\n"
return 0
